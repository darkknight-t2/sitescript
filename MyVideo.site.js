...
function SecretCrypter() {
}


SecretCrypter.prototype = {
    _sbox: null,
    _mykey: null,
    
    decrypt: function ( src, key ) {
        _sbox = new Array( 255 );
        _mykey = new Array( 255 );
        var srcChar = this._hexToChars( src );
        var keyChar = this._strToChars( key );
        var result = this._calculate( srcChar, keyChar );
        
        return this._charsToStr( result );
    },
    
    _initialize: function ( pwd ) {
        var length = pwd.length;
        
        for ( var i = 0; i <= 255; i++ ) {
            _mykey[ i ] = pwd[ i % length ];
            _sbox[ i ] = i;
        }
        
        var num = 0;
        for ( var i = 0; i <= 255; i++ ) {
            num = (num + _sbox[ i ] + _mykey[ i ] ) % 256;
            var tmp = _sbox[ i ];
            _sbox[ i ] = _sbox[ num ];
            _sbox[ num ] = tmp;
        }
    ...
    _calculate: function ( plaintxt, psw ) {
        this._initialize( psw );
        var num1 = 0;
        var num2 = 0;
        var array = new Array();
        var tmp2 = undefined;
        ...
        for ( var i = 0; i < plaintxt.length; i++ ) {
            num1 = ( num1 + 1 ) % 256;
            num2 = ( num2 + _sbox[ num1 ] ) % 256;
            tmp1 = _sbox[ num1 ];
            _sbox[ num1 ] = _sbox[ num2 ];
            _sbox[ num2 ] = tmp1;
            
            var num3 = ( _sbox[ num1 ] + _sbox[ num2 ] ) % 256;
            tmp2 = _sbox[ num3 ];
            result = plaintxt[ i ] ^ tmp2;
            array.push( result );
        }
    ...
    _hexToChars: function ( hex ) {
        var chars = new Array();
        var str = hex.substr( 0, 2 ) == "0x" ? 2 : 0;
        
        while ( str < hex.length ) {
            chars.push( parseInt( hex.substr( str, 2 ), 16 ) );
            str = str + 2;
        }
        return chars;
    },
...
function isSiteUrl( url ) {
    if ( url.match( /http:\/\/www\.myvideo\.de\/watch\/\d+\/.*/ ) ) {
        return true;
    }
}
...
function getVideoDetail( url ) {
    ...
    text.match( /<h1 class='globalHd'>(.*?)<\/h1>/ );
    var title = RegExp.$1;
    
    text.match( /p\.addVariable\('_videoid','(\d+)'\)/ );
    var videoid = RegExp.$1;
    
    text.match( /p\.addVariable\('_encxml','(.*?)'\)/ );
    var encXml = RegExp.$1;
    
    var md5 = new MD5();
    var hashId = md5.getHash( videoid.toString() );
    
    var key = "c8407a08b3c71ea418ec9dc662f2a56e40cbd6d5a114aa50fb1e1079e17f2b83" + hashId;
    var hashKey = md5.getHash( key );
    
    var sec = new SecretCrypter();
    var str = sec.decrypt( encXml, hashKey );
    
    var xml = decodeURIComponent( str.replace( /\+/g, '%20' ) );
    
    xml.match( / SERVER='(.*?)'/ );
    var server = RegExp.$1;
    
    xml.match( /FLV='(.*?)'/ );
    var realUrl = server + RegExp.$1;
