...
function isSiteUrl( url ) {
    if ( url.match( /http:\/\/(www\.acfun\.cn|222.243.146.200)\/html\/.*\/\d+\/\d+\.html/ ) ) {
        return true;
    }
}
...
function getVideoDetail( url ) {
    ...
    text.match( /player\d?\.swf\?id=(\d+)/ );
    var id = RegExp.$1;
    
    var xmlUrl = "http://v.iask.com/v_play.php?vid=" + id;
