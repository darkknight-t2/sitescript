...
function UnescapeSJIS( str ) {
    return str.replace( /%(8[1-9A-F]|[9E][0-9A-F]|F[0-9A-C])(%[4-689A-F][0-9A-F]|%7[0-9A-E]|[@-~])|%([0-7][0-9A-F]|A[1-9A-F]|[B-D][0-9A-F])/ig
        , function( s ) {
            ...
            l = s.length;
            return 3 == l ? String.fromCharCode( c < 160 ? c : c + 65216 )
                          : JCT11280.charAt( ( c < 160 ? c - 129 : c - 193 ) * 188 + ( 4 == l ? s.charCodeAt( 3 ) - 64
...
function getVideoDetail( url ) {
    ...
    url.match( /http:\/\/video\.fc2\.com\/a?\/?content\/(.*?)\/(.*?)\// );
    var dataUrl = "http://video.fc2.com/ginfo.php?onLoad=%5Btype%20Function%5D&otag=undefined&playid=undefined&playlistid=undefined&upid=";
    dataUrl += RegExp.$2;
    
    var tilte = RegExp.$1;
    var type = GetEscapeCodeType( tilte );
    
    if ( type == "SJIS" ) {
        title = UnescapeSJIS( tilte );
    }
    else if ( type == "UTF8" ) {
        title = decodeURIComponent( tilte );
    }
    else {
        return null;
    }
