// ==SiteScript==
// @siteName    Jokeroo
// @siteUrl     http://www.jokeroo.com/video/
// @author      DarkKnight
...
function isSiteUrl( url ) {
    if ( url.match( /http:\/\/www\.jokeroo\.com\/.*video.*\/.*\.html/ ) ) {
        return true;
    }
}
...
function getVideoDetail( url ) {
    ...
    text.match( /<h1 class="titlevideo">\s*(.*?)\s*<\/h1>/ );
    var title = RegExp.$1;
    
    text.match( /param value=&quot;xml=(.*?)&quot;/ );
    var xmlUrl = "http://www.jokeroo.com/api/get/file/" + RegExp.$1 + "?jokeroo";
    
    var md5 = new MD5();
    var randString = md5.getHash( new String( Math.random() ) ).substr( 0, 8 );
    xmlUrl += "&r=" + randString;
    xmlUrl += "&k=" + md5.getHash( randString + RegExp.$1 + "joke" ).substr( 2, 8 );
    ...
    text.match( /<playlist url="(.*?)"/ );
    var realUrl = RegExp.$1;
    
    if ( text.match( /<extData a="(.*?)"/ ) != null ) {
        var extData = md5.getHash( RegExp.$1 + "jokeyJoke" ).substr( 2, 4 );
        realUrl += "&k2=" + extData;
    }
